...
async function publishPlan(plan, adminId) { if (!plan.length) return { publishedCount: 0, failedCount: 0, posts: [] }; const res = await fetch(`${SITE_ORIGIN.replace(/\/$/, '')}/api/admin/wechat-auto-publish`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-admin-user-id': adminId }, body: JSON.stringify({ syncMode: 'managed_market', deactivateMissing: true, runDate: DATE_KEY, posts: plan }) }); const text = await res.text(); let data = null; try { data = text ? JSON.parse(text) : null } catch { data = text } if (!res.ok || !data?.success) throw new Error(data?.error?.message || text || 'Failed to publish auto posts'); return data.data }

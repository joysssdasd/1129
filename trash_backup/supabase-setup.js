...
async function createExecFunction() {
    const createFunctionSQL = `
        ...
        EXCEPTION WHEN OTHERS THEN
            ...
            result := '{"success": false, "error": "' || SQLERRM || '"}'::JSON;
            RETURN result;
        END;
        $$;
    `;
    ...
    if (error) {
        console.error('❌ 创建 exec_sql 函数失败:', error);
        return false;
    }

    console.log('✅ exec_sql 函数创建成功');
    return true;
}

async function main() {
    try {
        console.log('🚀 老王开始连接Supabase...');

        // 先创建 exec_sql 函数
        const functionCreated = await createExecFunction();
        if (!functionCreated) {
            console.error('❌ 无法创建 exec_sql 函数，退出');
            return;
        }
...
module.exports = { setupDatabase, createExecFunction };
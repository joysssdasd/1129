...
console.log('🔧 老王开始连Supabase...');
console.log('URL:', supabaseUrl);
console.log('Key:', supabaseKey ? '已配置' : '使用默认值');

const supabase = createClient(supabaseUrl, supabaseKey);

// 直接执行创建payment_qrcodes表的所有SQL
async function setupPaymentQRCodesTable() {
    ...
    try {
        ...
        if (insertError) {
            console.error('❌ 插入默认数据失败:', insertError);
            return false;
        }
...
if (require.main === module) {
    setupPaymentQRCodesTable().then(success => {
        if (success) {
            console.log('\n🎊 所有任务完成！payment_qrcodes表已成功创建！');
        } else {
            console.log('\n❌ payment_qrcodes表设置失败！');
            process.exit(1);
        }
    }).catch(err => {
...
module.exports = { setupPaymentQRCodesTable };